...
// クイズ用猫動画リスト
const QUIZ_CAT_VIDEOS = CAT_VIDEOS

// 欲求発見の説明セリフデータ
const desireDiscoveryExplanationLines: DialogueLine[] = [
  {
    icon: "💖",
    title: "欲求とは、あなたの行動を駆り立てる根本的なもの",
    content: "「お金が欲しい」「成功したい」という表面的な欲求の奥には、「安心したい」「認められたい」という本当の欲求があります。"
  },
  {
    icon: "🔍",
    title: "「なんで？」を繰り返すことで、本当の欲求が見える",
    content: "表面的な欲求に「なんで欲しいの？」と問いかけ続けることで、心の奥にある本当の願いが浮かび上がってきます。"
  },
  {
    icon: "🌟",
    title: "本当の欲求を知ると、満たし方が変わる",
    content: "例えば「安心したい」という欲求は、お金だけでなく、信頼できる人間関係や自己肯定感でも満たせます。"
  }
]
    ...
    >
      <div className="w-full max-w-2xl space-y-6 pb-20">
        ...
        <div className="text-center py-4">
          <p className="text-xs tracking-widest text-pink-600 uppercase mb-2 opacity-80">
            {subtitle || "Desire Discovery"}
          </p>
            ...
            {lines.map((_, index) => (
              <div
                ...
                className={`w-3 h-3 rounded-full transition-all ${
                  index === currentLineIndex ? "bg-pink-500 w-6" : index < currentLineIndex ? "bg-pink-400" : "bg-gray-300"
                }`}
              />
            ))}
          </div>
        ...
        <div
          onClick={handleClick}
          className="cursor-pointer bg-white rounded-2xl p-6 shadow-lg border-4 border-pink-400 relative overflow-hidden transition-transform hover:scale-[1.01] active:scale-[0.99]"
          style={{
        ...
        >
          {/* 上部グラデーションバー */}
          <div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-pink-500 to-purple-400" />
          ...
          <div className="absolute bottom-4 right-4">
            <span
              className={`text-pink-500 text-2xl ${!isTyping ? "animate-pulse" : "opacity-30"}`}
            >
              ▼
            </span>
          </div>
...
// 20個の質問用画像リスト（質問は動的に生成）
const quizImages: { PC: string[]; スマホ: string[] }[] = [
  { PC: ["/image/worthho1 (2).png"], スマホ: ["/image/worth1"] },
  { PC: ["/image/worthho2.png"], スマホ: ["/image/worth52"] },
  { PC: ["/image/worthho3.png"], スマホ: ["/image/worth31"] },
  { PC: ["/image/worthho4.png"], スマホ: ["/image/worth41"] },
  { PC: ["/image/worthho5.png"], スマホ: ["/image/worth51"] },
  { PC: ["/image/worthho6.png"], スマホ: ["/image/worth6"] },
  { PC: ["/image/worthho7.png"], スマホ: ["/image/worth7"] },
  { PC: ["/image/worthho8.png"], スマホ: ["/image/worth8"] },
  { PC: ["/image/worthho9.png"], スマホ: ["/image/worth9"] },
  { PC: ["/image/worthho10.png"], スマホ: ["/image/worth10"] },
  { PC: ["/image/worthho11.png"], スマホ: ["/image/worth11"] },
  { PC: ["/image/worthho12.png"], スマホ: ["/image/worth12"] },
  { PC: ["/image/worthho13.png"], スマホ: ["/image/worth13"] },
  { PC: ["/image/worthho14.png"], スマホ: ["/image/worth14"] },
  { PC: ["/image/worthho15.png"], スマホ: ["/image/worth15"] },
  { PC: ["/image/worthho16.png"], スマホ: ["/image/worth16"] },
  { PC: ["/image/worthho17.png"], スマホ: ["/image/worth17"] },
  { PC: ["/image/worthho18.png"], スマホ: ["/image/worth18"] },
  { PC: ["/image/worthho19.png"], スマホ: ["/image/worth19"] },
  { PC: ["/image/worthho20.png"], スマホ: ["/image/worth20"] },
]

// 動的な質問を生成する関数
const getDynamicQuestion = (currentQuiz: number, allUserAnswers: string[][]): string => {
  // 最初の質問
  if (currentQuiz === 0) {
    return "あなたが欲しいものは何ですか？"
  }
  ...
  if (previousAnswers.length > 0) {
    ...
    return `「${answersText}」が欲しいのはなんでですか？`
  }

  return "それが欲しいのはなんでですか？"
}

// 動的な回答例を生成する関数
const getDynamicAnswerExample = (currentQuiz: number): string => {
  if (currentQuiz === 0) {
    return "例：お金、愛、成功、健康、幸せ、自由、認められること"
  }
  return "例：安心したいから、幸せになりたいから、自分を認めたいから、大切な人を守りたいから"
}
...
const LoadingPage = ({ onLoadComplete }: { onLoadComplete: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-purple-400 via-pink-400 to-red-400 flex flex-col items-center justify-center z-50">
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        <p className="text-white/80 mb-8 text-lg">心の奥にある本当の願いを見つける</p>
...
const IntroPage = ({ onStart, isMuted, setIsMuted }: { onStart: () => void; isMuted: boolean; setIsMuted: (value: boolean) => void }) => {
  ...
  return (
    <div className="relative w-full min-h-screen flex flex-col items-center justify-center p-6 animate-fade-in" style={{ background: "linear-gradient(135deg, #fce7f3 0%, #fbcfe8 50%, #f9a8d4 100%)" }}>
      <TermsOfService isOpen={isTermsOpen} onClose={() => setIsTermsOpen(false)} />
      <div className="w-full max-w-2xl space-y-6">
        {/* タイトルカード */}
        <div className="bg-white rounded-2xl p-8 shadow-lg border-4 border-pink-400 relative overflow-hidden">
          <div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-pink-500 to-purple-400" />

          <div className="text-center mb-6">
            <p className="text-xs tracking-widest text-pink-600 uppercase mb-2 opacity-80">Desire Discovery</p>
            ...
            {/* 猫動画サムネイル */}
            <div className="flex justify-center mt-4">
              <div className="w-20 h-20 rounded-full overflow-hidden border-4 border-pink-400 shadow-lg">
                <video
                  src={explainCatVideo}
                  className="w-full h-full object-cover"
                  muted
          ...
          <div className="space-y-4 text-gray-700">
            <p className="text-lg leading-relaxed">
              「あなたが欲しいもの」から始めて、「なんで欲しいのか？」を繰り返し問いかけていきます。
              その奥にある<span className="text-pink-600 font-bold">本当の願い</span>を見つけましょう。
            </p>
        ...
        <div className="text-center space-y-3">
          <p className="text-gray-500 text-sm">
            スタートボタンをおすと、<button type="button" onClick={() => setIsTermsOpen(true)} className="text-pink-600 underline hover:text-pink-800 font-medium">利用規約</button>に同意したことになります。
          </p>
          <button
            onClick={onStart}
            className="bg-gradient-to-r from-pink-500 to-purple-500 text-white font-black px-12 py-4 rounded-full text-xl tracking-wide shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all"
          >
            スタート →
          </button>
        </div>
    ...
    <div className="relative w-full min-h-screen flex flex-col items-center justify-center p-6 animate-fade-in" style={{ background: "linear-gradient(135deg, #fce7f3 0%, #fbcfe8 50%, #f9a8d4 100%)" }}>
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl p-8 shadow-lg border-4 border-pink-400 relative overflow-hidden">
          ...
          <h1 className="text-2xl font-bold text-pink-800 mb-6 text-center">はじめに</h1>
          ...
          <div className="space-y-6">
            ...
            <div className="pt-4">
              <Button
                ...
                className="w-full bg-pink-500 hover:bg-pink-600 text-white py-3 text-lg disabled:opacity-50 disabled:cursor-not-allowed"
              >
                次へ
              </Button>
            </div>
...
const QuizPage = ({
  ...
  // 境界チェック
  if (currentQuiz >= maxQuestions || currentQuiz >= quizImages.length) {
    return (
      <div className="relative w-full h-screen flex items-center justify-center" style={{ background: "linear-gradient(135deg, #fce7f3 0%, #fbcfe8 50%, #f9a8d4 100%)" }}>
        <div className="text-center">
          <h2 className="text-2xl font-bold text-pink-800 mb-4">クイズ完了！</h2>
          <Button onClick={onDesireFound} className="bg-pink-500 hover:bg-pink-600 text-white">
            結果を見る
          </Button>
        </div>
      </div>
  ...
  // 動的に質問を生成
  const currentQuestion = getDynamicQuestion(currentQuiz, allUserAnswers)
  const currentAnswerExample = getDynamicAnswerExample(currentQuiz)
  ...
  return (
    <div className="relative w-full min-h-screen flex flex-col" style={{ background: "linear-gradient(135deg, #fce7f3 0%, #fbcfe8 50%, #f9a8d4 100%)" }}>
      {/* Confetti Animation */}
        ...
        <div className="flex-1 flex flex-col items-center p-4 md:p-6 pb-32">
                ...
                {Array.from({ length: 20 }, (_, i) => (
                  <div
                    ...
                    className={`w-2 h-2 rounded-full transition-all ${
                      i === currentQuiz ? "bg-pink-500 w-4" : i < currentQuiz ? "bg-pink-400" : "bg-gray-300"
                    }`}
                  />
                ))}
              </div>
          ...
          <div className="w-full max-w-2xl">
            <div className={`bg-white rounded-2xl p-6 shadow-lg border-4 relative overflow-hidden ${
              isGoldenQuestion(currentQuiz) ? "border-yellow-400" : "border-pink-400"
            }`}>
...
const SummaryPage = ({
  ...
  return (
    <div className="relative w-full min-h-screen flex flex-col items-center p-6 animate-fade-in" style={{ background: "linear-gradient(135deg, #fce7f3 0%, #fbcfe8 50%, #f9a8d4 100%)" }}>
      <ConfettiCanvas isActive={showConfetti} duration={1000} particleCount={30} points={300} />

      <div className="w-full max-w-2xl space-y-6">
        {/* タイトルカード */}
        <div className="bg-white rounded-2xl p-6 shadow-lg border-4 border-pink-400 relative overflow-hidden">
          <div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-pink-500 to-purple-400" />

          <h1 className="text-2xl font-bold text-pink-800 mb-2 text-center" style={{ fontFamily: "'Kosugi Maru', sans-serif" }}>
            💖 あなたの本当の欲求を選択してください
          </h1>
          ...
          {/* 猫イラスト */}
          <div className="flex justify-center mt-4">
            <div className="w-16 h-16 rounded-full overflow-hidden border-4 border-pink-400 shadow-lg">
              <video
                src={summaryCatVideo}
                className="w-full h-full object-cover"
                muted
              ...
              return (
                <button
                  ...
                  className={`w-full p-4 rounded-xl text-left transition-all duration-200 ${
                    isSelected
                      ? "bg-pink-500 text-white shadow-lg transform scale-[1.02]"
                      : canSelect
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {isSelected && <div className="w-3 h-3 bg-pink-500 rounded-full" />}
                    </div>
        ...
        <div className="space-y-4">
          ...
          <Button
            onClick={onExit}
            className="w-full text-pink-600 bg-transparent hover:text-pink-700 hover:bg-pink-50 border-0"
          >
            終了
          </Button>
        </div>
...
const ActionPlanPage = ({
  ...
  if (showIntro) {
    return (
      <div className="relative w-full min-h-screen flex flex-col items-center p-6 animate-fade-in overflow-y-auto" style={{ background: "linear-gradient(135deg, #fce7f3 0%, #fbcfe8 50%, #f9a8d4 100%)" }}>
        <div className="w-full max-w-2xl space-y-6 pb-20">
          <div className="text-center py-4">
            <p className="text-xs tracking-widest text-pink-600 uppercase mb-2 opacity-80">Action List</p>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-800 leading-tight" style={{ fontFamily: "'Kosugi Maru', sans-serif" }}>
              🎯 行動リストの作成
            </h1>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-lg border-4 border-pink-400 relative overflow-hidden">
            <div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-pink-500 to-purple-400" />
            <h2 className="text-pink-600 font-bold text-lg mb-4 flex items-center gap-2" style={{ fontFamily: "'Kosugi Maru', sans-serif" }}>
              <span>📝</span> これからやること
            </h2>
            <div className="space-y-4">
              <div className="flex gap-4 bg-pink-50 p-4 rounded-xl">
                <span className="text-3xl">🎯</span>
                <div>
                  <p className="font-bold text-base text-gray-800 mb-2">欲求に沿った行動をリストアップ</p>
                  <p className="text-sm text-gray-600 leading-relaxed">
                    あなたが見つけた本当の欲求に基づいて、<span className="text-pink-600 font-semibold">具体的な行動プラン</span>を作成します。
                  </p>
                </div>
              </div>
              <div className="flex gap-4 bg-pink-50 p-4 rounded-xl">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 leading-relaxed">
                    各レベルの行動について、<span className="text-pink-600 font-semibold">「いつ」「どこで」「何をするか」</span>を具体的に書きます。
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-lg border-4 border-purple-400 relative overflow-hidden">
            <div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-purple-500 to-pink-400" />
            ...
            <div className="space-y-2">
              {selectedAnswers.map((answer, index) => (
                <div key={index} className="flex items-center gap-3 p-3 bg-purple-50 rounded-xl">
                  <span className="bg-purple-500 text-white font-bold min-w-[28px] h-7 rounded-full flex items-center justify-center text-sm">
                    {index + 1}
                  </span>
                  <span className="text-purple-900 font-medium">{answer}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="text-center py-6">
            <button
              ...
              className="bg-gradient-to-r from-pink-500 to-purple-500 text-white font-black px-12 py-4 rounded-full text-base tracking-wide shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all"
            >
              行動リストを作成する →
            </button>
          </div>
  ...
  if (currentStep === 'confirmation') {
    return (
      <div className="relative w-full min-h-screen flex flex-col bg-gradient-to-b from-pink-50 to-pink-100 overflow-y-auto">
        <ConfettiCanvas isActive={showConfetti} duration={1500} particleCount={50} points={currentPoints} />

        <div className="flex-1 flex flex-col items-center justify-start p-4">
          <div className="w-full max-w-md mb-2">
            <div className="h-3 bg-gray-200 rounded-full overflow-hidden">
              <div className="h-full bg-pink-500 transition-all duration-500" style={{ width: '100%' }} />
            </div>
          </div>
            ...
            {[1, 2, 3, 4, 5, 6].map((level) => (
              <div
                ...
                className={`w-10 h-10 rounded-full transition-all duration-300 flex items-center justify-center ${
                  level <= completedLevels + 1 ? 'bg-pink-500 shadow-lg shadow-pink-500/50' : 'bg-gray-300'
                }`}
              >
          ...
          <div className="text-pink-600 text-2xl font-extrabold mb-4">🏆 {totalPoints}pt</div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-lg w-full max-w-md mb-4">
            ...
            <div className="bg-pink-50 rounded-lg p-4 mb-4">
              <p className="text-gray-700"><span className="font-bold text-purple-700">いつ：</span>{currentPlan.when}</p>
              <p className="text-gray-700"><span className="font-bold text-purple-700">どこで：</span>{currentPlan.where}</p>
              <p className="text-gray-700"><span className="font-bold text-purple-700">何を：</span>{currentPlan.what}</p>
            </div>
            ...
            <div className="space-y-3">
              {currentLevel < 6 && (
                <button
                  ...
                  className="w-full bg-gradient-to-r from-pink-500 to-purple-500 text-white font-bold py-3 rounded-full shadow-lg hover:shadow-xl transition-all"
                >
  ...
  return (
    <div className="relative w-full min-h-screen flex flex-col bg-gradient-to-b from-pink-50 to-pink-100 overflow-y-auto">
          ...
          >
            <div className="bg-white rounded-2xl p-4 shadow-2xl max-w-sm w-full mx-4">
              <video
                ...
                onEnded={() => setIsVideoPlaying(false)}
              />
              <p className="text-center text-pink-600 font-bold mt-2">+{currentPoints}pt 獲得！</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex-1 flex flex-col items-center justify-start p-4">
        <div className="w-full max-w-md mb-2">
          <div className="h-3 bg-gray-200 rounded-full overflow-hidden">
            <div className="h-full bg-pink-500 transition-all duration-500" style={{ width: `${getProgress()}%` }} />
          </div>
        </div>
            ...
            <div
              ...
              className={`w-10 h-10 rounded-full transition-all duration-300 flex items-center justify-center ${
                level < currentLevel
                  ...
                  : level === currentLevel
                    ? 'bg-pink-400 shadow-md animate-pulse'
                    : 'bg-gray-300'
              }`}
            >
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg w-full max-w-md mb-4">
          <div className="bg-pink-100 rounded-xl p-4 mb-4 relative">
            <div className="absolute -top-2 left-6 w-4 h-4 bg-pink-100 transform rotate-45" />
            <p className="text-pink-800 font-bold text-lg">{getBubbleText()}</p>
          </div>

          <div className="bg-purple-50 rounded-lg p-3 mb-4">
            <p className="text-sm text-purple-600 font-medium">あなたの欲求：</p>
            <p className="text-purple-800 font-bold">{selectedAnswers[0]}</p>
          </div>
          ...
          {(isCustomMode || currentStep === 'what') && (
            <div className="space-y-4">
              <Input
                ...
                className="w-full p-4 text-lg border-2 border-pink-300 rounded-xl focus:border-pink-500 focus:ring-pink-500"
              />
            </div>
          )}
    ...
    <div className="relative w-full min-h-screen flex flex-col items-center p-6 animate-fade-in" style={{ background: "linear-gradient(135deg, #fce7f3 0%, #fbcfe8 50%, #f9a8d4 100%)" }}>
      <div className="w-full max-w-2xl space-y-6">
        <div className="bg-white rounded-2xl p-8 shadow-lg border-4 border-pink-400 relative overflow-hidden">
          ...
          <h1 className="text-3xl font-bold text-pink-800 mb-6 text-center">あなたの欲求診断結果</h1>

          <div className="text-center mb-6">
            <div className="text-5xl font-bold text-pink-700 mb-2">🏆 {totalPoints}pt</div>
              ...
              <div className="space-y-2">
                {selectedAnswers.map((answer, index) => (
                  <div key={index} className="flex items-start gap-3 p-3 bg-white rounded-lg shadow-sm">
                    <span className="bg-pink-500 text-white font-bold min-w-[28px] h-7 rounded-full flex items-center justify-center text-sm flex-shrink-0">
                      {index + 1}
                    </span>
                    <span className="text-pink-900 font-medium">{answer}</span>
                  </div>
                ))}
              </div>
            </div>
                ...
                {actionPlans.map((plan, index) => (
                  <div key={index} className="bg-white rounded-lg p-4 shadow-sm">
                    <div className="flex items-start gap-3">
                      <span className="bg-purple-500 text-white font-bold min-w-[28px] h-7 rounded-full flex items-center justify-center text-sm flex-shrink-0">
                        {index + 1}
                      </span>
                      <div className="flex-1">
                        <p className="text-gray-700 mb-1"><span className="font-bold text-purple-700">いつ:</span> {plan.when || "未入力"}</p>
                        <p className="text-gray-700 mb-1"><span className="font-bold text-purple-700">どこで:</span> {plan.where || "未入力"}</p>
                        <p className="text-gray-700"><span className="font-bold text-purple-700">何を:</span> {plan.what || "未入力"}</p>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {!hasSubmitted ? (
            ...
            <div className="border-t-2 border-gray-200 pt-6 text-center">
              <p className="text-xl text-pink-600 font-bold">✓ アンケートにご協力ありがとうございました！</p>
            </div>
          )}
        ...
        <div className="flex gap-4 justify-center">
          <Button onClick={onRestart} className="bg-pink-500 hover:bg-pink-600 text-white px-8 py-3">
            もう一度診断する
          </Button>
...
export default DesireDiscoveryGame
